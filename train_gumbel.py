...
class Config:
    ...
    # Anti-Shortcut
    mi_target: float       = 8.0         # Целевой лосс для слепого Студента (снижено, чтобы Учитель не уходил в полную абстракцию)
...
class Logger:
    ...
    def update(self, loss, s_ce, s_kl, t_ce, t_kl, ent_local, ent_global, mi_ce, step_t):
        self.s["loss"]  += loss
        self.s["s_ce"]  += s_ce
        self.s["s_kl"]  += s_kl
        self.s["t_ce"]  += t_ce
    ...
    for _ in range(9999):
        for batch in loader:
            ...
            with accelerator.accumulate(teacher, student):
                ...
                # Экспоненциальный штраф-стена: если mi падает ниже mi_target (8.0),
